...
class AdminEditForm extends React.Component {
    ...
    componentDidMount() {
        App.api('/admin/roles').then((roles) => {
            ...
            if (roles.length === 0) {
                ...
                let {id} = this.state;
                if (id !== 0) {
                    App.api('/admin/admin', {id}).then((admin) => {
                        this.setState({admin})
                    })
                }
            }
    ...
    handleSubmit = () => {
        ...
        App.api('/admin/save_admin', {'admin': JSON.stringify(admin)}).then((res) => {
            window.history.back();
        });
    };
        ...
        return <div className="common-edit-page">
            ...
            <Card
                title={<BreadcrumbCustom
                    first={<Link to={CTYPE.link.admin.path}>{CTYPE.link.admin.txt}</Link>}
                    second='编辑管理员'/>}
